import { render, screen } from "@testing-library/react";
import { TodoList } from "./TodoList";

const mockTodos = [
	{
		id: 1,
		todo: "Do something nice for someone you care about",
		completed: false,
		userId: 152,
	},
	{
		id: 2,
		todo: "Memorize a poem",
		completed: true,
		userId: 13,
	},
	{
		id: 3,
		todo: "Watch a classic movie",
		completed: true,
		userId: 68,
	},
	{
		id: 4,
		todo: "Watch a documentary",
		completed: false,
		userId: 84,
	},
	{
		id: 5,
		todo: "Invest in cryptocurrency",
		completed: false,
		userId: 163,
	},
	{
		id: 6,
		todo: "Contribute code or a monetary donation to an open-source software project",
		completed: false,
		userId: 69,
	},
	{
		id: 7,
		todo: "Solve a Rubik's cube",
		completed: true,
		userId: 76,
	},
	{
		id: 8,
		todo: "Bake pastries for yourself and neighbor",
		completed: true,
		userId: 198,
	},
	{
		id: 9,
		todo: "Go see a Broadway production",
		completed: false,
		userId: 7,
	},
	{
		id: 10,
		todo: "Write a thank you letter to an influential person in your life",
		completed: true,
		userId: 9,
	},
	{
		id: 11,
		todo: "Invite some friends over for a game night",
		completed: false,
		userId: 104,
	},
	{
		id: 12,
		todo: "Have a football scrimmage with some friends",
		completed: false,
		userId: 32,
	},
	{
		id: 13,
		todo: "Text a friend you haven't talked to in a long time",
		completed: true,
		userId: 2,
	},
	{
		id: 14,
		todo: "Organize pantry",
		completed: false,
		userId: 46,
	},
	{
		id: 15,
		todo: "Buy a new house decoration",
		completed: true,
		userId: 105,
	},
	{
		id: 16,
		todo: "Plan a vacation you've always wanted to take",
		completed: true,
		userId: 162,
	},
	{
		id: 17,
		todo: "Clean out car",
		completed: false,
		userId: 71,
	},
	{
		id: 18,
		todo: "Draw and color a Mandala",
		completed: true,
		userId: 6,
	},
	{
		id: 19,
		todo: "Create a cookbook with favorite recipes",
		completed: true,
		userId: 53,
	},
	{
		id: 20,
		todo: "Bake a pie with some friends",
		completed: false,
		userId: 162,
	},
	{
		id: 21,
		todo: "Create a compost pile",
		completed: false,
		userId: 13,
	},
];

describe("TodoList component", () => {
	it("should show loading state initially", () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({ todos: mockTodos }),
		} as any);
		render(<TodoList />);

		expect(screen.getByText(/loading/i)).toBeInTheDocument();
	});

	it("should render todos after successful fetch", async () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({ todos: mockTodos }),
		} as any);

		render(<TodoList />);

		expect(screen.getByText(/loading/i)).toBeInTheDocument();
		for (const item of mockTodos) {
			expect(await screen.findByText(item.todo)).toBeInTheDocument();
		}
		expect(screen.queryByText(/loading/i)).not.toBeInTheDocument();
	});

	it("should show 'No result!' when API returns empty todos array", async () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({ todos: [] }),
		} as any);

		render(<TodoList />);

		expect(await screen.findByText(/no result!/i)).toBeInTheDocument();
	});

	it("should show 'No result!' when API returns no todos property", async () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({}),
		} as any);

		render(<TodoList />);

		expect(screen.getByText(/loading/i)).toBeInTheDocument();
		expect(await screen.findByText(/no result!/i)).toBeInTheDocument();
	});

	it("should show 'No result!' when fetch fails", async () => {
		jest.spyOn(globalThis, "fetch").mockRejectedValueOnce(
			new Error("Network Error")
		);

		render(<TodoList />);

		expect(screen.getByText(/loading/i)).toBeInTheDocument();
		expect(await screen.findByText(/no result!/i)).toBeInTheDocument();
	});

	it("should render todos in ul with li elements", async () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({ todos: mockTodos }),
		} as any);

		const { container } = render(<TodoList />);

		for (const item of mockTodos) {
			expect(await screen.findByText(item.todo)).toBeInTheDocument();
		}

		const ulElement = container.querySelector("ul");
		const liElements = container.querySelectorAll("li");

		expect(ulElement).toBeInTheDocument();
		expect(liElements).toHaveLength(mockTodos.length);
	});

	it("should call fetch with correct URL", async () => {
		jest.spyOn(globalThis, "fetch").mockResolvedValueOnce({
			json: async () => ({ todos: [] }),
		} as any);

		render(<TodoList />);

		expect(globalThis.fetch).toHaveBeenCalledWith(
			"https://dummyjson.com/todos"
		);
	});
});
